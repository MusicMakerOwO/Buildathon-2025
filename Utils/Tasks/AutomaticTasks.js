...
const TASKS = [
	// [ 'task_name', require("./TaskFile"), interval_in_seconds	]
];
...
module.exports.StartTasks = async function StartTasks() {
	...
	for (let i = 0; i < TASKS.length; i++) {
		...
		const lastRun = await selectQuery.pluck().get(name) || 0;
		...
		TaskScheduler.schedule(() => {
			try {
				callback();
				Database.prepare("INSERT INTO Timers (id, last_run) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET last_run = excluded.last_run").run(name, Date.now());
			} catch (err) {
				Log('ERROR', err);
			}
		}, delay, interval);
