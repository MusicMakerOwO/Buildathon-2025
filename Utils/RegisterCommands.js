...
async function RegisterCommands(client) {
	...
	for (let i = 0; i < localCommands.length; i++) {
		...
		try {
			...
			Log('ERROR', `[REGISTER] Failed to register ${command.data.name}: ${error}`);
		}
	}

	if (devCommands.length > 0 && !config.DEV_GUILD_ID) {
		Log('WARN', `You have dev commands but no DEV_GUILD_ID in config.json - These will not be registered!`);
	}
...
module.exports = async function DynamicRegister(client, force = false) {
	...
	if (!config.REGISTER_COMMANDS) {
		Log('WARN', 'Command registration is disabled in config.json');
		return;
	}
	...
	Log('INFO', 'Checking slash commands, this may take a second...');
	const oldCommands = {};
	const newCommands = {};
	...
	for (let i = 0; i < localCommands.length; i++) {
		...
		if (typeof command?.data !== 'object') {
			Log('ERROR', `Command ${command.name} has no data object`);
			continue;
		}
	...
	if (needToRegister) {
		...
		Log('INFO', 'No changes detected in commands');
	}
}